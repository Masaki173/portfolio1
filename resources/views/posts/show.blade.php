...
<a href="/{{$post->category_code}}" class="category-url">社会</a>
@elseif($post->category_code === 3)
<a href="/{{$post->category_code}}" class="category-url">カルチャー</a>
@elseif($post->category_code === 4)
<a href="/{{$post->category_code}}" class="category-url">テクノロジー</a>
@elseif($post->category_code === 5)
<a href="/{{$post->category_code}}" class="category-url">エンタメ</a>
@endif
@if($post->is_fee === 1 && !$post->is_subscribed_by_auth_user() && $post->user->id !== Auth::user()->id )
@auth
